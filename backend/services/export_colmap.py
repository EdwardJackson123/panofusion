...
def run_mixed_export(out_dir=None):
    ...
    # Keep the Y-up coordinate system set by align_ground_plane.main().
    # chunk.crs.localframe() would rotate to ENU (Z-up), which breaks Three.js Y-up rendering.
    # Instead, center coords at region origin in geocentric space, preserving Y-up orientation.
    fr_to_gc = chunk.transform.matrix
    if chunk.region:
        center_gc = fr_to_gc.mulp(chunk.region.center)
        T_shift = Metashape.Matrix.Translation(-center_gc) * fr_to_gc
    elif chunk.tie_points:
        valid = [pt for pt in chunk.tie_points.points if pt.valid]
        if valid:
            avg = Metashape.Vector([sum(pt.coord[i] / pt.coord[3] for pt in valid) / len(valid) for i in range(3)])
            center_gc = fr_to_gc.mulp(avg)
            T_shift = Metashape.Matrix.Translation(-center_gc) * fr_to_gc
        else:
            T_shift = fr_to_gc
    else:
    ...
    # COLMAP standard convention: +X=right, +Y=down, +Z=forward.
    # Our aligned internal frame is Y-up; flip Y,Z for COLMAP compliance so that
    # external tools (LichtFeld Studio, COLMAP GUI, Postshot) see the expected axes.
    colmap_axes = Metashape.Matrix.Diag([1, -1, -1, 1])
    T_shift = colmap_axes * T_shift
    colmap_cams = {}
    colmap_imgs = []
    points3d_list = {}
    sensor_map = {}
