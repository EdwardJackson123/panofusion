...
# The worker runs inside Metashape's bundled Python, which may lack numpy/cv2.
# Inject the packaged Python's site-packages so both align_ground_plane and
# export_colmap can import them without a separate installation.
_BACKEND_DIR = Path(__file__).resolve().parent           # backend/services/
_PYTHON_LIB = _BACKEND_DIR.parent.parent / "python" / "Lib" / "site-packages"
if _PYTHON_LIB.exists() and str(_PYTHON_LIB) not in sys.path:
    sys.path.insert(0, str(_PYTHON_LIB))
